...
export class BillSaleDetailComponent implements OnInit {
  ...
  caculatorQuote() {
    ...
    this.billSaleModel.listBillSaleDetail.forEach(item => {
      ...
      item.vatValue = (item.quantity * item.exchangeRate * item.unitPrice - discount) * (item.vat / 100);
      this.TotalSumAmountProduct = this.TotalSumAmountProduct + item.quantity * item.exchangeRate * item.unitPrice - discount;
