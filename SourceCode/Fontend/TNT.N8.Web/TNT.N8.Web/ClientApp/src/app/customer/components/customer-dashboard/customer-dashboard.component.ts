...
export class CustomerDashboardComponent implements OnInit {
  ...
  goToListCustomerIdentification() {
    this.router.navigate(['/customer/list', {mode : 'Identification'}]);
  }

  goToListCustomerFree() {
    this.router.navigate(['/customer/list', {mode : 'Free'}]);
  }
  goToListOrder() {
  ...
  createBarChart(array: Array<any>, element: number) {
    ...
    if (element == 1) {
      ...
      const maxwidthofLabel = 13 - (labels.length > 10 ? 10 : labels.length);
      let _data = [];
      labels.forEach(l => {
        _data.push({
